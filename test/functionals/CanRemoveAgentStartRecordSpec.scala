...
class CanRemoveAgentStartRecordSpec extends Specification with InjectorSupport with UsingSelenide {
  override val conf: Map[String, Any] = inMemoryDatabase()

  "Agent record" should {
    "Can remove start agent record" in new WithServer(app = appl, port = testPort) {
      inject[Database].withConnection { implicit conn =>
        ...
        $(".clearAgentRecordConfirm .dialogButtons .clearEndAgentRecord").click()
        $$("table.score tbody tr").shouldHaveSize(1)
        $$("table.score tbody tr").get(0).find(".agentName").text === "shanai"

        $("#onlyOrphanRecord input").click()
        $$("table.score.orphan tbody tr").shouldHaveSize(1)
        $$("table.score tbody tr").get(0).find(".agentName").text === "shanai2"
      }
    }
  }
